...
- Мелочи (не блокеры): карточки, добавленные ДО пина на испанский Испании, остались
  со старой лексикой/транскрипцией (можно сделать разовый пере-проход); inline-кнопка
  «Показать ответ» в карточках нажимается повторно.
  Отложено финальным ревью ветки language-profiles (2026-08-04, все triaged
  OK-TO-DEFER): недетерминизм `llm_key_map` при коллизии ключей в ответе модели
  (страхует response_schema; фикс — mapped-ключи поверх исходных через update);
  DI-контракт (`dp["profile"]` ↔ имя параметра `profile`) не закреплён тестом;
  `card_preview` проверяется подстроками, а не golden-string (байт-идентичность
  es-вывода в CI не заперта); таблица `cards` в старой спеке
  `2026-06-02-spanish-bot-design.md` осталась со старыми именами колонок.
