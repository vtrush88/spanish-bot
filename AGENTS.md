...
- **edge-tts отдаёт MP3**, а Telegram `answer_voice` хочет OGG/OPUS. Поэтому озвучка
  идёт через **`answer_audio`** (mp3), file_id кэшируется (`sent.audio.file_id`). Не
  переключай на answer_voice без конвертации в opus.
...
- Вне MVP (в спеке): пуш-напоминания, статистика прогресса, распознавание речи мамы.
